...
const calculate = () => {
	fetch(`https://api.exchangerate.host/latest?base=${currencyOne.value}&symbols=${currencyTwo.value}`)
		.then(res => res.json())
		.then(data => {
			const currency1 = currencyOne.value // przechowują wartość pól
			const currency2 = currencyTwo.value

			const rate = data.rates[currency2] // przechowuje wartość jako aktualny kurs walut
